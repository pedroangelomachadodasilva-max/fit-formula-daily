import { Droplets, Scale, Flame, Coffee, BookOpen, Camera, Dumbbell, TrendingUp, ShoppingBag, Check, Plus } from "lucide-react";
    ...
    <div className="screen-content space-y-4 animate-fade-in">
      ...
      <div className="card-highlight">
        <div className="flex items-start justify-between">
          ...
          <span className="text-4xl">{todayTea.image}</span>
        </div>
        <div className="flex gap-2 mt-3">
          <button
            onClick={() => setActiveTab("tea")}
            className="flex-1 text-xs font-medium py-2 rounded-xl bg-card text-foreground"
          >
            Ver receita
          </button>
          <button
            ...
            className={`flex-1 text-xs font-medium py-2 rounded-xl flex items-center justify-center gap-1 ${
              state.dailyLog.teasDrunk.includes(todayTea.id)
      ...
      <div className="card-elevated">
        ...
        {showWeightInput ? (
          <div className="flex gap-2 mt-3">
            ...
            />
            <button
              onClick={() => { if (weightInput) { setWeight(parseFloat(weightInput)); setShowWeightInput(false); setWeightInput(""); } }}
